...
public class StationHandler {
	...
	private static Vector2f[] largeStationLocations = { new Vector2f(133, -85), new Vector2f(200, -376),
			new Vector2f(226, -741), new Vector2f(794, -442), new Vector2f(623, -608) };
	...
	public static List<SupplyStation> checkSonar(Submarine sub, float scale, List<SupplyStation> sonarStations) {
		for (SupplyStation station : supplyStations) {
			if (sub.getPosition().x + 20 * (scale) > station.getPosition().x - station.getScale().x / 2
					&& sub.getPosition().x - 20 * (scale) < station.getPosition().x + station.getScale().x / 2) {
				if (sub.getPosition().y + 20 * (scale) > station.getPosition().y - station.getScale().y / 2
						&& sub.getPosition().y - 20 * (scale) < station.getPosition().y + station.getScale().y / 2) {
	...
	public static void resetRespawn() {
		StationHandler.respawn = null;
	}

	public static SupplyStation getRespawn() {
		return StationHandler.respawn;
	}
